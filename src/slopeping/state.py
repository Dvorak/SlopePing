...
def _stable_part(part: Any) -> str:
    if isinstance(part, list):
        return ",".join(str(item).strip() for item in part)
    return str(part).strip()


def _record_from_dict(data: dict[str, Any]) -> ScheduleRecord:
    raw_actions = data.get("available_actions", [])
    available_actions = raw_actions if isinstance(raw_actions, list) else []
    return ScheduleRecord(
        ...
        confirmation_status=str(data.get("confirmation_status", "unknown")),
