...
def parse_overview_records(page: Page, selectors: Selectors) -> list[ScheduleRecord]:
    return [parsed.record for parsed in parse_overview_rows(page, selectors)]


def parse_overview_rows(page: Page, selectors: Selectors) -> list[ParsedScheduleRow]:
...
def _record_from_row(headers: dict[int, str], cells: list[str], row: Locator) -> ScheduleRecord:
    values = {
        "tag": "",
        "von": "",
        "bis": "",
...
def _confirmation_cell(row: Locator, headers: dict[int, str]) -> Locator | None:
    for index, field in headers.items():
        if field == "bestaetigung":
...
def _confirmation_details(raw_text: str, cell: Locator | None) -> tuple[str, list[str]]:
    if cell is not None:
        actions = _select_options(cell)
        selectable_actions = [action for action in actions if action]
        if {"Bestätigen", "Absagen"}.issubset(set(selectable_actions)):
            return "pending", selectable_actions

    normalized = _normalize_text(raw_text).casefold()
    if "bestätigt" in normalized or "bestaetigt" in normalized:
        return "confirmed", []

    return "unknown", []


def _select_options(cell: Locator) -> list[str]:
    options = cell.locator("select option")
    return [_normalize_text(options.nth(index).inner_text()) for index in range(options.count())]
