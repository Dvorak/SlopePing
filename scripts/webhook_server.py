...
def check_configuration() -> bool:
    ...
    if webhook_token:
        print(
            f"  ✓ ACTION_WEBHOOK_TOKEN: {'*' * len(webhook_token)} (configured)",
            flush=True,
        )
    else:
...
def print_startup_info() -> None:
    ...
    print(
        "     → Then SlopePing logs in, finds the lesson, clicks action, saves",
        flush=True,
    )
    print("", flush=True)
    ...
    print(
        "  ⚠ The control page still uses a URL token - use trusted networks or HTTPS",
        flush=True,
    )
